...
def convert_input(row):
    ...
    input_s += ' <extra_id_1> '+ ' <extra_id_0> '.join(history)
#     input_s += ' <extra_id_1> '.join(history)
...
output_path = f"{model_name}-lif-mtl-follow-qr-current_q_first"
...
get_ipython().system('ls t5-base-lif-mtl-follow-qr/checkpoint-3142')
...
for chk in os.listdir('t5-base-lif-mtl-follow-qr/'):
    if not chk.startswith('checkpoint'):
        continue
    print(chk)
    model.load_state_dict(torch.load('t5-base-lif-mtl-follow-qr-current_q_first/'+ chk + '/pytorch_model.bin'))
    model.to(device)
    model.eval()
    preds = []
    lbls = [s.split() for s in df_test['qr'].to_list()]
...
model.load_state_dict(torch.load('t5-base-lif-mtl-follow-qr-current_q_first/checkpoint-12568/pytorch_model.bin'))
model.to(device)
model.eval()
# preds = []
qid2qr = {}
